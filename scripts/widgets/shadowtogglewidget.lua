...
function ShadowToggleWidget:OnClickButton()
    ...
    if ThePlayer.hasshadowsenabled == true then
        ThePlayer:PushEvent("TurnOnShadows")
    end
    if ThePlayer.hasshadowsenabled == false then
        ThePlayer:PushEvent("TurnOffShadows")
    end
end
...
function ShadowToggleWidget:OnUpdate(dt)
    ...
    if self.backpack and self.backpack.replica.container and self.backpack.replica.container._isopen and Profile:GetIntegratedBackpack() == true then
